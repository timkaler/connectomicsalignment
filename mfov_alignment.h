...
// Assumes that 2d alignment has already been done.
void better_mfov_align(Graph<vdata, edata>* merged_graph, align_data_t* p_align_data) {
  // 3 points in each MFOV for 3d alignment.
  // point in overlap connects adjacent tiles in 2d.

  // Need to prevent it from collapsing everything to a point. 
  // In triangle, spring is (delta_s1 - original_length)**2


  Graph<vdata, edata>* mesh_graph = new Graph<vdata, edata>();
  mesh_graph->resize(merged_graph->num_vertices());

 
  std::map<int, std::vector<int> > adjacent_tiles_2d; 
  for (int v = 0; v < merged_graph->num_vertices(); v++) {
    vdata* vd = merged_graph->getVertexData(v);
    *mesh_graph->getVertexData(v) = *vd;
    cv::Point2f vpoint = merged_graph->getVertexData(v)->original_center_point;
    for (int i = 0; i < merged_graph->edgeData[v].size(); i++) {
      ...
      if (v < nid) {
        mesh_graph->insert_matches(v, nid, npoints, vpoints, 1.0);
      }
    }
  }
  
  mfov_alignment_3d(mesh_graph, p_align_data);

  for (int v = 0; v < mesh_graph->num_vertices(); v++) {
    vdata* vd = mesh_graph->getVertexData(v);
    *merged_graph->getVertexData(v) = *vd;
  }
}
