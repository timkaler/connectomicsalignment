...
void fine_alignment_3d_dampen_remove(Graph<vdata, edata>* merged_graph, align_data_t* p_align_data){
    printf("STARTING THE FINE ALIGNMENT IT SO FINE (I hope)");
    if (true) {
    //int vertex_id_offset = 0;
...
void fine_alignment_3d_2(Graph<vdata, edata>* merged_graph, align_data_t* p_align_data, double ransac_thresh){
    ...
    for (int section = 1; section < p_align_data->n_sections; section++) {
      ...
      std::vector< cv::Point2f > filtered_match_points_b(0);
        int section_a = section-1;
        int section_b = section;
      for (double box_iter_x = min_x; box_iter_x < max_x+15000; box_iter_x += 12000) {
      for (double box_iter_y = min_y; box_iter_y < max_y+15000; box_iter_y += 12000) {
        // Filter the matches with RANSAC
                int num_filtered = 0;
        std::vector<cv::Point2f> match_points_a, match_points_b;
        double box_min_x = box_iter_x;//min_x + (max_x-min_x)*box_iter_x*1.0/20;
        double box_max_x = box_iter_x+12000;//min_x + (max_x-min_x)*(box_iter_x+1)*1.0/20;
        double box_min_y = box_iter_y;//min_y + (max_y-min_y)*box_iter_y*1.0/20;
        double box_max_y = box_iter_y+12000;//min_y + (max_y-min_y)*(box_iter_y+1)*1.0/20;
